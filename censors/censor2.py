    ...
    def check_censor(self, packet):
        ...
        try:
            ...
            if packet["TCP"].sprintf('%TCP.flags%') == "S":
                ...
                self.logger.debug("Synchronizing TCB on packet: %s", geneva.layers.packet.Packet._str_packet(packet))
                return False
            ...
            if packet["TCP"].seq == self.tcb:
                ...
                self.logger.debug("Ignoring packet: %s", geneva.layers.packet.Packet._str_packet(packet))
                return False
