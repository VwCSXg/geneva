...
class Censor11(Censor):
    ...
    def check_censor(self, packet):
        ...
        """
        try:
            self.logger.debug("Inbound packet to censor: %s" % geneva.layers.packet.Packet._str_packet(packet))
            ...
            if (tcb and self.resynchronize[(tcb["src"], tcb["dst"], tcb["sport"], tcb["dport"])]) or \
               (not tcb and packet["TCP"].sprintf('%TCP.flags%') in ["S", "A"]):
                ...
                if tcb is None:
                    self.logger.debug("Making a new TCB for packet %s" % geneva.layers.packet.Packet._str_packet(packet))
                    tcb = {}
                ...
                self.logger.debug("Synchronizing a TCB (%s) on packet %s " % (str(tcb), geneva.layers.packet.Packet._str_packet(packet)))
                return False
            ...
            # If connection is getting torn down
                ...
                self.logger.debug(("Entering resynchronization state on packet %s" % geneva.layers.packet.Packet._str_packet(packet)))
            ...
            for keyword in self.forbidden:
                if keyword in self.get_payload(packet):
                    self.logger.debug("Packet triggered censor: %s" % geneva.layers.packet.Packet._str_packet(packet))
                    return True
    ...
    def get_matching_tcb(self, packet):
        ...
        for tcb in self.tcbs:
            self.logger.debug("Checking %s against packet %s" % (str(tcb), geneva.layers.packet.Packet._str_packet(packet)))
    ...
    def get_partial_tcb(self, packet):
        ...
        """
        for tcb in self.tcbs:
            self.logger.debug("Checking %s against packet %s for partial match" % (str(tcb), geneva.layers.packet.Packet._str_packet(packet)))
