    ...
    def check_censor(self, packet, verbose=False):
        ...
        try:
            ...
            if "S" == packet["TCP"].sprintf('%TCP.flags%'):
                ...
                self.logger.debug(("Synchronizing TCB (%d) on S packet " + geneva.layers.packet.Packet._str_packet(packet)) % self.tcb)
                return False

            if "A" == packet["TCP"].sprintf('%TCP.flags%'):
                self.tcb = packet["TCP"].seq
                self.logger.debug(("Synchronizing TCB (%d) on A packet " + geneva.layers.packet.Packet._str_packet(packet)) % self.tcb)
                return False
