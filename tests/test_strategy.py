...
def test_mate(logger):
    ...
    assert not geneva.actions.strategy.mate(strat1, strat2, 1)
    ...
    geneva.actions.strategy.mate(strat1, strat2, 1)
...
def test_init(logger):
    ...
    strat = geneva.actions.strategy.Strategy([], []).initialize(logger, 1, 0, 1, 0, None)
    s = "[TCP:flags:R]-drop-| \/"
    # initialize with a seed
    assert str(geneva.actions.strategy.Strategy([], []).initialize(logger, 1, 1, 1, 1, s)).strip() == s
...
def test_run(logger):
    ...
    p1 = geneva.layers.packet.Packet(IP(src="127.0.0.1", dst="127.0.0.1") / TCP(sport=2222, dport=3333, seq=100, ack=100, flags="S"))
    ...
    p1 = geneva.layers.packet.Packet(IP(src="127.0.0.1", dst="127.0.0.1") / TCP(sport=2222, dport=3333, seq=100, ack=100, flags="A", dataofs=5))
    ...
    p1 = geneva.layers.packet.Packet(IP(src="127.0.0.1", dst="127.0.0.1") / TCP(sport=2222, dport=3333, seq=100, ack=100, flags="A", dataofs=5, chksum=100))
...
def test_sleep_parse_handling(logger):
    ...
    assert not geneva.actions.sleep.SleepAction().parse("THISHSOULDFAIL", logger)

    assert geneva.actions.sleep.SleepAction().parse("10.5", logger)
...
def test_fail_cases(logger):
    ...
    p = geneva.layers.packet.Packet(IP(src="127.0.0.1", dst="127.0.0.1") / TCP(sport=2222, dport=3333, seq=100, ack=100, flags="S") / Raw("aaaaaaaaaa"))
